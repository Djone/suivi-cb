...
exports.getAllSubCategories = async (req, res) => {
    try {
        ...
        console.log('Sous-catégories récupérées :', categories);
...
exports.getAllSubCategoriesByfinancielFlowId = async (req, res) => {
    try {
        ...
        const categories = await subCategory.getAllByFinancialFlowId(financialFlowId);
        console.log('Sous-catégories récupérées :', categories);
        res.status(200).json(categories);
    } catch (err) {
        console.error('Erreur lors de la récupération des sous-catégories :', err);
...
exports.updateSubCategory = async (req, res) => {
    try {
        ...
        console.log('ID reçu :', id); // Log de l'ID
        console.log('Données reçues pour mise à jour :', req.body); // Log des données du corps

        if (!id) {
            return res.status(400).json({ error: 'ID de la sous-catégorie manquant.' });
        }
        ...
        res.status(200).json({
            message: 'Sous-catégorie mise à jour avec succès.', updateData,
            changes: result.changes,
        });
    } catch (err) {
        console.error('Erreur lors de la mise à jour de la sous-catégorie :', err.message || err);
        res.status(500).json({
            error: 'Erreur interne du serveur lors de la mise à jour de la sous-catégorie.',
        });
    }
};
