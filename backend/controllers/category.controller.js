...
exports.getActiveCategories = async (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des catégories actives :",
      err
    );
    res.status(500).json({ error: "Erreur interne du serveur" });
  }
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    res.status(500).json({
      error:
        "Erreur interne du serveur lors de la mise à jour de la catégorie.",
    });
  }
};
...
exports.deleteCategoryById = async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ error: "ID invalide pour la désactivation." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la désactivation de la catégorie." });
  }
};
...
exports.reactivateCategory = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la réactivation de la catégorie." });
  }
};
