...
exports.updateTransaction = async (req, res) => {
  try {
    ...
    console.log(
      "Données reçues pour mise à jour de la transaction:",
      fieldsToUpdate
    ); // Log des données du corps
    ...
    res.status(500).json({
      error:
        "Erreur interne du serveur lors de la mise à jour de la transaction.",
    });
  }
};
...
exports.deleteTransactionById = async (req, res) => {
  ...
  if (!id) {
    return res
      .status(400)
      .json({ error: "ID invalide pour la suppression de la transaction." });
  }
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la suppression de la transaction." });
  }
};
