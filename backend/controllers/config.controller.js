...
exports.getAccountsConfig = (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la lecture de la configuration des comptes:",
      err
    );
    res.status(500).json({
      error: "Erreur lors de la lecture de la configuration des comptes",
    });
  }
};
...
exports.updateAccountsConfig = (req, res) => {
  try {
    ...
    if (!accounts || !Array.isArray(accounts)) {
      return res
        .status(400)
        .json({ error: "Format de configuration invalide" });
    }
    ...
    console.error(
      "Erreur lors de la mise à jour de la configuration des comptes:",
      err
    );
    res.status(500).json({
      error: "Erreur lors de la mise à jour de la configuration des comptes",
    });
  }
};
