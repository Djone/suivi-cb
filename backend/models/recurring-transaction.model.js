...
const RecurringTransaction = {
  ...
  getAll: () => {
    return new Promise((resolve, reject) => {
      db.all(
        "SELECT * FROM recurring_transactions ORDER BY is_active DESC, day_of_month ASC",
        [],
        (err, rows) => {
          if (err) {
            reject(err);
  ...
  add: (recurringTransaction) => {
    ...
    const values = columns.map((col) => {
      if (col === "frequency") {
        return recurringTransaction[col] || "monthly";
      }
      if (Object.prototype.hasOwnProperty.call(recurringTransaction, col)) {
        return recurringTransaction[col];
      }
      return null;
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      db.run(query, values, function (err) {
        if (err) {
          logger.error(
            "Erreur lors de la création de la transaction récurrente"
          );
          reject(err);
        } else {
  ...
  update: (recurringTransaction) => {
    ...
    return new Promise((resolve, reject) => {
      db.run(query, values, function (err) {
        ...
        if (this.changes === 0) {
          return reject(
            new Error("Aucune transaction récurrente trouvée avec cet ID.")
          );
        }
  ...
  deleteById: (id) => {
    return new Promise((resolve, reject) => {
      if (!id) {
        return reject(
          new Error(
            "ID manquant pour la suppression de la transaction récurrente."
          )
        );
      }
  ...
  reactivate: (id) => {
    return new Promise((resolve, reject) => {
      if (!id) {
        return reject(
          new Error(
            "ID manquant pour la réactivation de la transaction récurrente."
          )
        );
      }

      const query =
        "UPDATE recurring_transactions SET is_active = 1 WHERE id = ?";
      db.run(query, [id], function (err) {
        if (err) {
          logger.error(
