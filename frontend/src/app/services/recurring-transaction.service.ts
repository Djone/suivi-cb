...
export class RecurringTransactionService {
  ...
  private recurringTransactionsSubject = new BehaviorSubject<
    ...
    return this.http.get<RecurringTransaction[]>(this.apiUrl).pipe(
      tap((recurringTransactions) => {
        const camelCaseData = recurringTransactions
          ...
          .map((rt) =>
            this.normalizeRecurringTransaction(rt as RecurringTransaction),
          );
      ...
      catchError((err) => {
        console.error(
          'RECURRING TRANSACTION SERVICE : Erreur lors du chargement:',
          err,
        );
        throw err;
      }),
  ...
  addRecurringTransaction(
    ...
    return this.http.post<void>(this.apiUrl, snakeCaseData).pipe(
      ...
      catchError((err) => {
        console.error(
          "RECURRING TRANSACTION SERVICE : Erreur lors de l'ajout:",
          err,
        );
        throw err;
      }),
  ...
  updateRecurringTransaction(
    ...
    return this.http.put<void>(url, snakeCaseData).pipe(
      ...
      catchError((err) => {
        console.error(
          'RECURRING TRANSACTION SERVICE : Erreur lors de la mise à jour:',
          err,
        );
        throw err;
      }),
  ...
  deleteRecurringTransaction(id: number): Observable<void> {
    ...
    return this.http.delete<void>(url).pipe(
      ...
      catchError((err) => {
        console.error(
          'RECURRING TRANSACTION SERVICE : Erreur lors de la désactivation:',
          err,
        );
        throw err;
      }),
  ...
  reactivateRecurringTransaction(id: number): Observable<void> {
    ...
    return this.http.patch<void>(url, {}).pipe(
      ...
      catchError((err) => {
        console.error(
          'RECURRING TRANSACTION SERVICE : Erreur lors de la réactivation:',
          err,
        );
        throw err;
      }),
  ...
  private normalizeRecurringTransaction(
  ...
  private parseActiveMonths(value: unknown): number[] | null {
    ...
    if (typeof value === 'string') {
      ...
      try {
        arrayCandidate = trimmed.startsWith('[')
          ? JSON.parse(trimmed)
          : trimmed.split(',');
      } catch {
        return null;
